...
export const useTodayPosts = (limitCount: number = 50, refreshKey: number = 0) => {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        ...
        todayPosts.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
        setPosts(todayPosts);
      } catch (error) {
        console.error('今日の投稿取得に失敗:', error);
      } finally {
        if (active) {
          setLoading(false);
        }
